					...
					if (nestedObj != null && prop.Name == "AnswerValue")
					{
						...
						if (nestedObj is string)
						{
							...
							if (!string.IsNullOrEmpty (nestedValue) && (nestedValue.StartsWith ("A_") || nestedValue.StartsWith ("B_") || nestedValue.StartsWith ("C_")))
							{
								...
								// "A_", "B_", "C_" 접두사 제거 -> Substring (2);
								while (nestedValue.StartsWith ("A_") || nestedValue.StartsWith ("B_") || nestedValue.StartsWith ("C_"))
								{
									nestedValue = nestedValue.Substring (2);
								}

								prop.SetValue (obj, nestedValue); // 처리
							}
						}
						else
						{
